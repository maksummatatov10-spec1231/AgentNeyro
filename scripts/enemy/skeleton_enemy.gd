...
## Скелет-враг (Этап 3, начало). Прямое преследование игрока + ближний бой.
## Анимации (Rig_Medium) — следующий шаг; пока поворачивается к игроку.
...
func _die() -> void:
	...
	# лёгкое падение/исчезновение
	var tw := create_tween()
	tw.tween_property(self, "scale", Vector3(0.01, 0.01, 0.01), 0.4).set_ease(Tween.EASE_IN)
